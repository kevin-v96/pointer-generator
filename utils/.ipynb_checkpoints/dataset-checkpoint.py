...
import logging
error_logger = tf.get_logger().setLevel(logging.ERROR)
warning_logger = tf.get_logger().setLevel(logging.WARN)
info_logger = tf.get_logger().setLevel(logging.INFO)
...
class Batcher(object):
    ...
    def __init__(self, data_path, vocab, batch_size, single_pass, mode):
        self._vocab = vocab
    ...
    def next_batch(self):
        ...
        if self._batch_queue.qsize() == 0:
            warnings.warn(f'Bucket input queue is empty when calling next_batch. Bucket queue size: {self._batch_queue.qsize()}, Input queue size: {self._example_queue.qsize()}')
            #warning_logger(
            ...
            if self.single_pass and self._finished_reading:
                warnings.warn("Finished reading dataset in single_pass mode.")
                #info_logger("Finished reading dataset in single_pass mode.")
                return None
    ...
    def fill_example_queue(self):
        ...
        while True:
            try:
                ...
                warnings.warn("The example generator for this example queue filling thread has exhausted data.")
                #info_logger("The example generator for this example queue filling thread has exhausted data.")
                if self.single_pass:
                    warnings.warn("single_pass mode is on, so we've finished reading dataset. This thread is stopping.")
                    #info_logger(
    ...
    def watch_threads(self):
        while True:
            warnings.warn(f'Bucket queue size: {self._batch_queue.qsize()}, Input queue size: {self._example_queue.qsize()}')
            #info_logger(
            ...
            for idx, t in enumerate(self._example_q_threads):
                if not t.is_alive():  # if the thread is dead
                    warnings.warn('Found example queue thread dead. Restarting.')
                    #error_logger('Found example queue thread dead. Restarting.')
            ...
            for idx, t in enumerate(self._batch_q_threads):
                if not t.is_alive():  # if the thread is dead
                    warnings.warn('Found batch queue thread dead. Restarting.')
                    #error_logger('Found batch queue thread dead. Restarting.')
    ...
    def pair_generator(self, example_generator):
        while True:
            ...
            try:
                ...
                warnings.warn('Failed to get article or abstract from example')
                #error_logger('Failed to get article or abstract from example')
                continue
